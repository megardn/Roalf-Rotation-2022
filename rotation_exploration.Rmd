...
dx_multi <- dx_multi %>% unite(DIAGNOSIS, c (dx_pscat, goassessDxpmr6), na.rm = TRUE, remove=FALSE) %>% 
  mutate(DIAGNOSIS = as.factor(DIAGNOSIS),
         DIAGNOSIS = fct_collapse(DIAGNOSIS, "healthy" = c("noDSMdx", "TD"),
                                "other_dx" = c("OP", "other"),
                                "GO_PS" = c("PS"),
                                "CAP_pro" = c("pro"),
                                "CAP_psych" = c("psy")))
...
**to do:** update dx categories to include pnc dx for colors, drop subj w/out at least 1 yr clinical follow up
