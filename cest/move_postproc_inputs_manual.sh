...
#B0
	for b0 in $(ls $dicoms/${sub}_${ses}/*BOMAP*)
    do
		if [ -f $base/${sub}_${ses}/${sub}-${ses}-B0map.nii ]
		then
		rm $base/${sub}_${ses}/${sub}-${ses}-B0map.nii
		fi 
		/project/bbl_projects/apps/melliott/scripts/dicom2nifti.sh -u -r Y -F $base/${sub}_${ses}/${sub}-${ses}-B0map.nii $b0
    done
#B1
    ...
    do
		...
		/project/bbl_projects/apps/melliott/scripts/dicom2nifti.sh -u -r Y -F $base/${sub}_${ses}/${sub}-${ses}-B1map.nii $b1
    done
...
#CEST
    ...
    do
		...
		/project/bbl_projects/apps/melliott/scripts/dicom2nifti.sh -u -r Y -F $base/${sub}_${ses}/${sub}-${ses}-B0B1CESTmap.nii $cest  
    done
