...
Seem to have some data for these ppts in spreadsheets David added/demographics from 7T list - merge in to pull protocol names/numbers and dates for when they may have gotten assessments and ask David for where diagnoses may be (or just other clinical data)
...
demo <- demo %>% select(-Sex_1M, -Ethnicity, -DOB)
...
Pulling "SIPS" ratings number of Pos, Neg, Gen, etc items on SOPS coded as 3,4,5, or 6 depending on suffix. Not planning to use these in final analyses, but already wrote the code.
...
#find smallest datediff that's >=365
post <- sips_scan %>%
  select(1, 7, 31:35, 48, 53) %>% #filter so we're not duplicating rows that wont change (demographics, 7t date, etc.)
  group_by(bblid) %>%
  filter(diff_in_days >= 365) %>%
  arrange(bblid, diff_in_days) %>%
  slice(c(1))%>% 
...
pre <- sips_scan %>%
  ...
  slice(c(1)) %>% 
  select(!c(abs_diff_in_days)) %>% 
  rename_at(c(7, 31:35, 48, 53), ~ paste0(.x, "_pre"))

#new dataframe
proximal_sips <- inner_join(pre, post, by = c("bblid" = "bblid_post")) %>%
  select(bblid, DOSIPS_pre, POS_345_pre, POS_6_pre, POS_3456_pre, NEG_3456_pre, DIS_3456_pre, GEN_3456_pre, diff_in_days_pre, dx, dx_source, dx_date, dx_diff, DOSIPS_post, POS_345_post, POS_6_post, POS_3456_post, NEG_3456_post, DIS_3456_post, GEN_3456_post, diff_in_days_post) %>%
...
* merge demo *actually* last and not by date so info gets applied to ALL timepoints

``` {r big.merge}
#making sure we have what we need/expect
str(dx_scan_date)
length(unique(dx_scan_date$bblid))
...
#renaming df and cols for compatibility across assessments
merge_base <- dx_scan_date %>% rename(date_assess = dx_date, diff_assess = dx_diff)

#clean and merge in cgas
summary(cgas)
cgas_clean <- cgas %>%
  mutate(bblid=as.factor(bblid),
...
#merge
merge_1 <- full_join(merge_base, cgas_clean, by = c("bblid", "date_assess"))
length(unique(merge_2$bblid)) #should always have 132 subj

#now sips clean & merge

# #try merging in demo first since it's relatively small
# summary(demo)
# merge_1 <- left_join(merge_base, demo, by = c("bblid" = "BBLID"))
# length(unique(merge_1$bblid)) #should always have 132 subj
```
