...
#filter df using P1 as indicator for completion
sips.sops_scan <- subset(sips_date, (bblid %in% clin365_sip$bblid & bblid %in% clin364_sop$bblid))

names(sips.sops_scan)

```

SOPS is subscale components of SIPS (which are overall ratings of severity).
Focusing on SOPS for now (summing within subscales Pos, Neg, Gen, Disorg) for now - seems to be valid scoring based on ct.gov, could also sum across.

``` {r sops}
#filter for SOPS completion
sops_scan <- sips.sops_scan %>% drop_na(P1) %>% 
  select(1, 7:26, 47, 48) %>% #subset columns
  mutate(sops_p = rowSums(across(3:7)),
         sops_n = rowSums(across(8:13)),
         sops_d = rowSums(across(14:17)),
         sops_g = rowSums(across(18:21)),
         sops_tot = rowSums(across(3:21))) #create subset composite scores
...
There are ` r length(unique(sops_scan$bblid))` (27) subjects with SOPS at baseline and at least 1 post-1 yr followup. 
...
summary(proximal_sops[55:59])
```

Pulling SIPS ratings; **ask Kosha about how these are coded.** 

``` {r sips}
#filter df for sips completion
sips_scan <- sips.sops_scan %>% drop_na(POS_345, NEG_3456, GEN_3456)

length(unique(sips_scan$bblid))
```

There are ` r length(unique(sips_scan$bblid))` (24) subjects with SIPS at baseline and at least 1 post-1 yr followup
...
Prime Screen scores were computed by totaling the number of items endorsed at the level of 5 or 6 and via raw sum of scores(SIP003 to SIP014) *check items* (per [Zak's paper](https://doi.org/10.1176/appi.ps.201800536) and [scale authors](10.1016/j.schres.2012.07.022))
