...
sips_clean <- sips_calc %>% select(1, 7:27, 58:61) %>% #sips_calc already filtered for correct source (proband>collateral) for each timepoint, frequency calculated
  ...
  subset(bblid %in% all_subj) #make sure correct subj list
#merge
merge_2 <- full_join(merge_1, sips_clean, by = c("bblid", "date_assess"))
...
#merge
merge_3 <- full_join(merge_2, prime_clean, by = c("bblid", "date_assess"))
length(unique(merge_3$bblid)) #should always have 132 subj


#clean syrp and merge
...
length(unique(merge_4$bblid)) #should always have 132 subj

# #try merging in demo first since it's relatively small
summary(demo)
demo_clean <- demo %>% subset(BBLID %in% all_subj) #make sure correct subj list
merge_5 <- inner_join(demo_clean, merge_4, by = c("BBLID" = "bblid"))
length(unique(merge_5$BBLID)) #should always have 132 subj
```
