...
cest.vals <- cest.rois %>%
  select_if(~!all(is.na(.))) %>% #drop any col that's totally empty
  mutate(scan = str_split(as.character(Subject), "/", simplify = T)[, 6]) %>% #get BBLID_scanID 
  select(-Subject)
...
```{r}
gm.vals %>% filter(!is.na(L_CerebralWM_numvoxels)) %>% select(scan, L_CerebralWM_numvoxels)
```
...
```{r}
gm.vals %>% filter(!is.na(R_CerebralWM_numvoxels)) %>% select(scan, R_CerebralWM_numvoxels)
```
