...
pacman::p_load(tidyverse, ggplot2, ggpubr, ggrepel, skimr, lubridate, tidyr, data.table, car, ggpubr, corrplot, lme4, lmerTest, report, DescTools, daff, GGally, atable, glmnet)
```
...
final.end_scaled <- final.df %>% filter(scan.time != "base.1") %>% mutate_if(is.numeric, scale)
```
...
Scanner is *not significantly* related to baseline diagnosis or baseline SOPS positive score but *is significantly* related to CEST contrast (averaged over thalamus), number of thalamic volumes captured, and age at scan. After controlling for scanner, number of voxels *is not* significantly associated with mean CEST value in the thalamus, therefore scanner (but not volume) will be controlled for in the final model.
...
```{r}
#plots
final.df %>% select(base_dx | contains("Thalamus")) %>% 
...
Overall, baseline diagnosis does not appear to be signficantly associated with thalamic cest when controlling for sex, age and scanner type, while scanner type does remain predictive.
...
ggplot(data = final.df_scaled_end, mapping = aes(x = R_Thalamus_mean, y = delta_sops)) +
...
The plot is vaguely suggestive of that in Egerton et al, but clearly not as significant. However, these are scaled raw values while their plot reported "Baseline thalamic glutamate level, adjusted for baseline attenuated positive symptom score".
...
Using scaled data so model will converge more readily. Building models with predictors chosen based on a priori subject knowledge and relationships shown above, getting sequentially more complex to avoid overfitting.
...
Using anova to compare model fit on chi-sq:

```{r}
anova(lmer.2, lmer.3, lmer.4)
```
...
```{r}
lmer.end <- lmer(sops_p ~ sips_diff_days*R_Thalamus_mean + R_Thalamus_numvoxels + age_sops + scanner + Sex_1M + base_sops_p + psych.rx_life + (1|BBLID), REML = T, data = final.df_scaled_end)

summary(lmer.end)
Anova(lmer.end)
```

nonrandom slopes on change

```{r}
lmer.change <- lmer(delta_sops ~ sips_diff_days + R_Thalamus_mean + R_Thalamus_numvoxels + age_sops + scanner + Sex_1M + base_sops_p + psych.rx_life + (1|BBLID), REML = T, data = final.df_scaled_end)
...
Binarized improvement (delta_sops < 0) vs worsening of symptoms (delta_sops > 0) - logistic regression

```{r}
final.end_scaled <- final.end_scaled %>%
  mutate(sops_worse = case_when(sops_cat == "remit" ~ 0,
                                sops_cat == "stable" ~ 0,
                                sops_cat == "progress" ~ 1))
#check if binarized correctly
#table(final.df_scaled_end$sops_cat, final.df_scaled_end$sops_worse)

fit.logit <- glmer(sops_worse ~ sips_diff_days + age_scan + Sex_1M + base_sops_p + (1|BBLID), final.end_scaled, family=binomial(logit))
summary(fit.logit)
Anova(fit.logit)
summary(rePCA(fit.logit))

#try prediction
fit.logit.predict <- predict(fit.logit, final.end_scaled, type="response") 

# #plot
# model_coefs <- coef(fit.logit)$BBLID %>% 
#   rename(Intercept = `(Intercept)`, Slope = sips_diff_days) %>% 
#   rownames_to_column("BBLID")
# 
# model1.data <- left_join(final.end, model_coefs, by="BBLID")
# 
# ggplot(data = model1.data, mapping = aes(x = sips_diff_days, y = sops_bin)) +
#   geom_point(na.rm = T, aes(col = BBLID)) +
#   geom_abline(aes(intercept = Intercept, 
#                   slope = Slope,
#                   colour = BBLID
