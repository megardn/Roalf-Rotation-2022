...
pacman::p_load(tidyverse, ggplot2, ggpubr, ggrepel, skimr, lubridate, tidyr, data.table, car, ggpubr, corrplot, lme4, lmerTest, report, DescTools, daff)
```
...
sips.scan.long.filt <- sips_scan_long.lab %>%
  group_by(BBLID, date7t) %>%
  arrange(abs(sips_diff_days)) %>%
...
table(as.factor(sips.scan.long.filt$scan.time))
n_unique(sips.scan.long.filt$BBLID)
```
...
scanlist_clean <- scanlist %>%
  ...
  select(-row)%>%
  ungroup() %>%
  mutate(observation = paste(bblid_scan, scantime),
         scanid=gsub(" ", "",paste(BBLID,"_",scanid)))

table(scanlist_clean$scantime)

scanlist_clean %>% filter(scantime=="base.1") %>%
  ggplot(aes(x=sips_diff_days)) + geom_dotplot()

skim(scanlist_clean)
```


Comparing to wide outputs 
```{r}
scanlist_clean.test <- scanlist_clean %>%
  rename(SCANID=scanid,
         scan.time=scantime) %>%
  mutate(observation=paste(BBLID, as.character(date7t), scan.time)) %>%
  select(-scan, -use.scan, -bblid_scan)

setdiff(scanlist_clean.test, sips.scan.long.filt)

render_diff(
    diff_data(data_ref = scanlist_clean.test,
              data = sips.scan.long.filt)
)
```
...
just_scans <- sops_demo %>% select(BBLID, observation, DOSIPS, date7t, scantime)
...
sops_dx.filt %>% 
  ggplot(aes(x=dx_sops_diff, fill=scantime)) + geom_bar() + facet_grid(cols = vars(scantime))

#focusing on baseline
sops_dx.filt %>% filter(scantime=="base.1") %>%
...
sops_dx.filt <- sops_dx.filt %>%
  select(-BBLID, -DOSIPS, -date7t, -scantime)
...
sops_dx.sum %>% 
  ggplot(aes(y=BBLID, x=scantime, color=diagnosis)) + geom_point()
```
...
```{r}
final.df <- full_join(sops_change_meds.life, r.thal.dat, by="scanid")
...
```{r}
ggplot(data = sops_change_meds.life, mapping = aes(x = sips_diff_days, y = sops_tot)) +
...
```{r}
lmer.1 <- lmer(sops_tot ~ sips_diff_days + (1|BBLID), REML = T, data = sops_change_meds.life)

model_coefs <- coef(lmer.1)$BBLID %>% 
  rename(Intercept = `(Intercept)`, Slope = sips_diff_days) %>% 
  rownames_to_column("BBLID")

model1.data <- left_join(sops_change_meds.life, model_coefs, by="BBLID")

ggplot(data = model1.data, mapping = aes(x = sips_diff_days, y = sops_tot)) +
  geom_point(na.rm = T, aes(col = BBLID)) +
...
```{r}
scaled.sops_change_meds.life <- sops_change_meds.life %>% mutate_if(is.numeric, scale)

lmer.2 <- lmer(sops_tot ~  (1 + sips_diff_days|BBLID), REML = F, data = scaled.sops_change_meds.life )
...
model2.data <- left_join(scaled.sops_change_meds.life, model_coefs2, by="BBLID")

ggplot(data = model2.data, mapping = aes(x = sips_diff_days, y = sops_tot)) +
  geom_point(na.rm = T, aes(col = BBLID)) +
  geom_abline(aes(intercept = Intercept, 
                  slope = Slope,
                  colour = BBLID
...
Boxplots of baseline dx vs various paramaeters

Looking at baseline diagnosis vs change GAF

```{r}
ggplot(sops_change_meds.life, aes(x=age_scan, y=slope.gafc, color=baseline.dx)) +
  geom_point() +
  xlab("Age at Scan") +
  ylab("GAF Slope") + 
  ggtitle("Change in GAF by Observation") + 
  theme_bw() + 
  theme(axis.text.x = element_text(angle = 30, hjust = 1))
```

```{r}
ggplot(sops_change_meds.life, aes(x=baseline.dx, y=slope.sops_tot, color=as.factor(psych.rx_life))) +
  geom_point() +
  geom_jitter(width = 0.25) +
  xlab("Baseline Dx") +
  ylab("Slope of Total SOPS") + 
  ggtitle("Change in SOPS by Observation") + 
  theme_bw() + 
  theme(axis.text.x = element_text(angle = 30, hjust = 1))
```
...
Playing around with [MEMs](https://yury-zablotski.netlify.app/post/mixed-effects-models-1/).

```{r}
toy.model = lmer(slope.sops_tot ~ baseline.dx + age_scan + (1|BBLID), data=sops_change_meds.life)
summary(toy.model)
report(toy.model)
```
