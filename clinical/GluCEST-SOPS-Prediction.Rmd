...
pacman::p_load(tidyverse, ggplot2, ggpubr, ggrepel, skimr, lubridate, tidyr, data.table, car, ggpubr, corrplot, lme4, lmerTest, report, DescTools, daff, GGally)
```
...
glu_old <- glu_old %>%
  ...
  select(-X7T_date, -ONM_7T_date, -ONM_SCANID)
...
just_scans <- sops_demo %>% select(BBLID, observation, DOSIPS, date7t, scan.time)
...
sops_dx.filt <- sops_dx.filt %>%
  select(-BBLID, -DOSIPS, -date7t, -scan.time)
...
```{r}
sops_dx.final %>% select(contains("AXIS1_DX")) %>% 
  t %>% c %>% unique
...
sops_dx.sum.filt <- sops_dx.sum %>%
  select(!contains("AXIS1") & !contains("HSTATUS") & !contains("CONSIDER") & !contains("PRODROMAL") & !contains("POSS_PRO_DX"))
```
...
```{r}
cest.rois <- read.csv("/Users/megardn/ImageData/PMACS_remote/data/cest_values/GluCEST-HarvardOxford-Subcortical-Measures.csv", header = TRUE, stringsAsFactors = F, sep = '\t', na="NaN")
skim(cest.rois)
...
#GM voxels
cest.gm.rois <- read.csv("/Users/megardn/ImageData/PMACS_remote/data/cest_values/GMDensity-HarvardOxford-Subcortical-Measures.csv", header = TRUE, stringsAsFactors = F, sep = '\t', na="NaN")
...
*NOTE: 1 SUBJECT HAS 2 SCANS, FILTER DOWN TO SCAN WITH BETTER QC OR MORE SOPS FOLLOWUPS*

one subject also weirdly has GM values in `L_CerebralWM_mean`, outputting scan ID so I can look at the niftis

```{r}
gm.vals %>% filter(!is.na(gm.L_CerebralWM_numvoxels)) %>% select(scanid, gm.L_CerebralWM_numvoxels)
```
...
```{r}
gm.vals %>% filter(!is.na(gm.R_CerebralWM_numvoxels)) %>% select(scanid, gm.R_CerebralWM_numvoxels)
```

Seems like having errant GM voxels (calculated probablistically via FAST) isn't uncommon in WM (as defined by HO Subcortical atlas), what makes 94276_10927 unique is slab orientation. **Will double check with David once final data is reprocessed.**

Merging cest values and filter to thalamic ROI

```{r}
all.cest <- merge(cest.vals, gm.vals, by="scanid")

r.thal.dat <- all.cest %>% select("scanid" | contains("R_Thalamus"))
skim(r.thal.dat)

r.thal.dat %>% select(-scanid) %>% ggpairs()
...
Look at CEST data by scanner - *LOOK AT SCORE DISTRIBUTION ACROSS BOTH SAMPLES*

```{r}
final.df %>% select(scanner | contains("Thalamus")) %>% ggpairs(aes(color=scanner))
```

Are CEST values or demographics different across scanners?

```{r}
#check for differences in baseline diagnoses
chisq.test(final.df$scanner, final.df$base_dx, simulate.p.value=TRUE)
#check for differences in scan age
chisq.test(final.df$scanner, final.df$age_scan, simulate.p.value=TRUE)
#check for differences in baseline SOPS Positive
final.df %>% filter(scan.time=="base.1")%>%
  t.test(sops_p ~ scanner, data=.)

#check for differences in thalamic CEST
t.test(R_Thalamus_mean ~ scanner, data = final.df)
#check for differences in thalamic volume captured
t.test(R_Thalamus_numvoxels ~ scanner, data = final.df)

#look for correlations between volume and CEST when controlling for scan
m.cest.vol <- lm(R_Thalamus_mean ~ scanner + R_Thalamus_numvoxels, data=final.df)
summary(m.cest.vol)
Anova(m.cest.vol)
```

Scanner is *not significantly* related to baseline diagnosis or baseline SOPS positive score but *is significantly* related to CEST contrast (averaged over thalamus), number of thalamic volumes captured, and age at scan. Even after controlling for scanner, number of voxels is significantly associated with mean CEST value in the thalamus: therefore both scanner and volume will be controlled for in the final model.
...
final.df %>% 
  group_by(BBLID) %>%
  sample_n(1) %>% #random selection
  ggplot(aes(x = diagnosis, y = sops_tot )) +
  geom_point(aes(color=as.factor(psych.rx_life)), show.legend = TRUE) +
  geom_jitter(width = 0.25) +
  theme(axis.text.x = element_text(angle = 45, hjust=1)) +
  ggtitle("SOPS Total Score vs Axis 1 Dx")
```
*not sure why some points are black, no `psych.rx_life` vals missing*
...
```{r}
final.df %>% select(diagnosis | contains("Thalamus")) %>% distinct() %>% ggpairs(aes(color=diagnosis))

final.df %>% 
  group_by(BBLID) %>%
  arrange(DOSIPS) %>% #get first timepoint for baseline dx
  slice_head() %>% #get first timepoint for baseline dx
  ungroup() %>%
  select(dx_control | contains("Thalamus")) %>% 
  distinct() %>% 
...
Lm to look for any interaction of thalamic CEST and diagnosis:
```{r}
lm.int <- lm(sops_p ~ R_Thalamus_mean + base_dx + R_Thalamus_mean*base_dx, data=final.df)
Anova(lm.int)

lm.int2 <- lm(sops_p ~ R_Thalamus_mean + R_Thalamus_numvoxels + R_Thalamus_mean*R_Thalamus_numvoxels, data=final.df)
Anova(lm.int2)
```


```{r}
lm.nonlin <- lm(sops_p ~  R_Thalamus_mean + R_Thalamus_numvoxels + base_dx + age_scan + scanner + Sex_1M + psych.rx_life + sips_diff_days + BBLID, data = final.df_scaled)
Anova(lm.nonlin)
plot(resid(lm.nonlin),final.df$sops_p)
plot(final.df$AGEONSET_CLINICRISK, final.df$sops_p)
```

Playing around with MEMs

```{r}
lmer.1 <- lmer(sops_p ~ sips_diff_days + (1|BBLID), REML = T, data = final.df)

model_coefs <- coef(lmer.1)$BBLID %>% 
  rename(Intercept = `(Intercept)`, Slope = sips_diff_days) %>% 
  rownames_to_column("BBLID")

model1.data <- left_join(final.df, model_coefs, by="BBLID")
...
**remove 1 scan for ppt with 2!**

Scaling data so model will converge

```{r}
lmer.2 <- lmer(sops_p ~  (1 + sips_diff_days|BBLID), REML = T, data = final.df_scaled )
...
ggplot(data = model2.data, mapping = aes(x = sips_diff_days, y = sops_p)) +
  geom_point(na.rm = T, aes(col = BBLID)) +
  geom_abline(aes(intercept = Intercept, 
                  slope = Slope,
                  colour = BBLID
...
```{r}
lmer.3 <- lmer(sops_p ~  R_Thalamus_mean + (sips_diff_days|BBLID), REML = T, data = final.df_scaled)

model_coefs3 <- coef(lmer.3)$BBLID %>% 
  rename(Intercept = `(Intercept)`, Slope = sips_diff_days) %>% 
  rownames_to_column("BBLID")

 relgrad <- with(lmer.3@optinfo$derivs,solve(Hessian,gradient))
 max(abs(relgrad))
...
```{r}
lmer.4 <- lmer(sops_p ~  R_Thalamus_mean + R_Thalamus_numvoxels + age_scan + scanner + Sex_1M + base_dx + (sips_diff_days|BBLID), REML = T, data = final.df_scaled) ## try correcting for scanner instead of gm vol?! try combat?
...
plot(lmer.4)
#normality - plotting quantlies
qqnorm(residuals(lmer.4))
```

Heteroscedastic - model fits less well at higher (and more important) sops_p scores

nonrandom slopes on endpoint data

```{r}
final.df_scaled_end <- final.df_scaled %>% filter(scan.time != "base.1")
lmer.end <- lmer(sops_p ~ sips_diff_days + R_Thalamus_mean + R_Thalamus_numvoxels + R_Thalamus_mean*R_Thalamus_numvoxels + age_sops + scanner + Sex_1M + base_sops_p + (1|BBLID), REML = T, data = final.df_scaled_end)
