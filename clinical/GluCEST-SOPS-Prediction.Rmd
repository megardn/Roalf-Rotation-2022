...
Plotting simple linear model for visualiation

```{r}
ggplot(data = final.df, mapping = aes(x = sips_diff_days, y = sops_tot)) +
...
```{r}
lmer.1 <- lmer(sops_tot ~ sips_diff_days + (1|BBLID), REML = T, data = final.df)
...
lmer.2 <- lmer(sops_tot ~  (1 + sips_diff_days|BBLID), REML = F, data = final.df_scaled )
...
```{r}
lmer.3 <- lmer(sops_tot ~  R_Thalamus_mean + sips_diff_days + gm.R_Thalamus_numvoxels + scanner + diagnosis + (1|BBLID), REML = T, data = final.df_scaled )
...
ggplot(data = model3.data, mapping = aes(x = sips_diff_days, y = sops_tot)) +
  geom_point(na.rm = T, aes(col = BBLID)) +
  geom_abline(aes(intercept = Intercept, 
                  slope = Slope,
                  colour = BBLID
