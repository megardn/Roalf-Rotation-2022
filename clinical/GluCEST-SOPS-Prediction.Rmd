...
Scanner is *not significantly* related to baseline diagnosis or baseline SOPS positive score but *is significantly* related to CEST contrast (averaged over thalamus), number of thalamic volumes captured, and age at scan. 
...
```{r}
lmer.4 <- lmer(sops_p ~  R_Thalamus_mean + gm.R_Thalamus_numvoxels + base_dx + age_scan + scanner + Sex_1M + psych.rx_life + (sips_diff_days|BBLID), REML = T, data = final.df_scaled) ## try correcting for scanner instead of gm vol?! try combat?
