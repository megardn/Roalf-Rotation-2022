...
pacman::p_load(tidyverse, ggplot2, ggpubr, ggrepel, skimr, lubridate, tidyr, data.table, car, ggpubr, corrplot, lme4, lmerTest, report)
```
...
sops_change_demo.phi <- left_join(sops_change, demo.phi, by="BBLID")
skim(sops_change_demo.phi)
n_unique(sops_change_demo.phi$observation)
```
...
```{r}
sops_change_demo <- sops_change_demo.phi %>%
  mutate(age_scan = as.numeric(difftime(date7t,DOB, units = "weeks"))/52.25,
         age_end = as.numeric(difftime(DOSIPS,DOB, units = "weeks"))/52.25) %>%
  select(-DOB)

summary(sops_change_demo$age_scan)
summary(sops_change_demo$age_end)
```
...
sops_change_dx.base <- sops_change_dx %>%
  ...
  rename_with(.cols = 40:63, function(x){paste0("base.", x)})
...
```{r}
bs.fit2 <- lm(slope.sops_tot ~ base.AXIS1_DX1 + age_scan, data=sops_change_dx.base)
Anova(bs.fit2)
```
...
scan_dates <- sops_change_dx.filt %>%
