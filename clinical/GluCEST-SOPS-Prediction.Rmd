...
pacman::p_load(tidyverse, ggplot2, ggpubr, ggrepel, skimr, lubridate, tidyr, data.table, car, ggpubr, corrplot, lme4, lmerTest, report, DescTools, daff, GGally, atable, glmnet, ppcor, plot3D, plot3Drgl, MetabolomicsBasics, performance, rsq)
```
...
**add table grouping `delta_sops` by diagnosis**
...
```{r}
cc <- scales::seq_gradient_pal("#6afec4", "#202236", "Lab")(seq(0,1,length.out=21))

ggplot(final.df, aes(x=sips_diff_days, y=sops_p, color=BBLID)) +
  geom_line() +
  xlab("Time from Scan") +
  ylab("SOPS Positive Score") + 
  ggtitle("SOPS-P Progression from Scan by Subject") + 
  theme_bw() +
  scale_colour_manual(values=cc)
```

```{r}
final.end %>% group_by(base_dx) %>%
   summarize(range = max(delta_sops) - min(delta_sops),
            median = median(delta_sops),
            mean = mean(delta_sops),
            sd = sd(delta_sops))

cc2 <- scales::seq_gradient_pal("#6afec4", "#f67c65", "Lab")(seq(0,1,length.out=4))

#plot of change by dx
final.end %>%
  ggplot() + 
  geom_histogram(aes(x = delta_sops, fill=base_dx), position="identity", alpha=0.75) +
  facet_wrap(~base_dx) +
...
```{r}
cc3 <- scales::seq_gradient_pal("#6afec4", "#202236", "Lab")(seq(0,1,length.out=4))
...
The plot is vaguely suggestive of that in Egerton et al, but clearly not significant. However, these are scaled raw values while their plot reported "Baseline thalamic glutamate level, adjusted for baseline attenuated positive symptom score". Attempting adjustment using suggested method [here](https://www.researchgate.net/post/How_to_interpret_adjusted_variables_in_linear_regression/5b03defbd6afb56c4071bcad/citation/download).
...
```{r, warning=FALSE}
scatter3D(final.end_scaled$sips_diff_days, final.end_scaled$R_Thalamus_mean, final.end_scaled$delta_sops, colvar = final.end_scaled$delta_sops, bty = "b2", col = NULL, add = FALSE, theta = 110, phi = 20, main = "", xlab = "Time", ylab ="Thalamic CEST", zlab = "Change in SOPS P")
plotrgl()
```
...
"Marginal and conditional r-squared values for mixed models are calculated based on Nakagawa et al. 2017... The marginal r-squared considers only the variance of the fixed effects, while the conditional rsquared takes both the fixed and random effects into account. The random effect variances are actually the mean random effect variances, thus the r-squared value is also appropriate for mixed models with random slopes or nested random effects (see Johnson 2014)."
...
Simple lm for visualization:

```{r}
lm.delta.5 <- lm(delta_sops ~ R_Thalamus_mean + scanner + Sex_1M + age_scan + base_sops_p + sips_diff_days + base_rx, final.end)
Anova(lm.delta.5)
summary(lm.delta.5)
#avPlots(lm.delta.5, terms=~.-scanner-Sex_1M-age_scan, col.lines="#f67c65", id=FALSE)
avPlots(lm.delta.5, terms="R_Thalamus_mean", col.lines="#f67c65", id=FALSE, ylab="Change in SOPS-P | Others", xlab="Thalamic CEST | Scanner, Sex, Age, Time, Baseline Psych Rx, Baseline SOPS-P", main="Partial Regression of Thalamic GluCEST vs Change in SOPS-P")


pcor.df2 <- end.df %>%
  mutate(scan.num = case_when(scanner == "Terra" ~ 0,
                                scanner == "ONM" ~ 1),
         base_rx = as.integer(base_rx)) %>% #must be numeric for pcor
  dplyr::select("delta_sops", "R_Thalamus_mean", "scan.num", "Sex_1M", "age_scan", "base_sops_p", "sips_diff_days", "base_rx")
pcor2 <- as.data.frame(ppcor::pcor(pcor.df2)$estimate) %>%
  dplyr::select(delta_sops) %>%
  mutate(var=as.factor(rownames(.))) %>%
  filter(var != "delta_sops")

pcor2$var <- recode_factor(pcor2$var,"base_sops_p" = "Baseline SOPS-P",
                           "scan.num" = "ONM Scanner",
                           "R_Thalamus_mean" = "Thalamic GluCEST",
                           "age_scan" = "Age at Scan",
                           "Sex_1M" = "Sex Male",
                           "sips_diff_days" = "Time (Days)",
                           "base_rx" = "Psych Rx")
pcor2$var <- factor(pcor2$var,                                    # Factor levels in decreasing order
                  levels = pcor2$var[order(pcor2$delta_sops, decreasing = FALSE)])

table(pcor2$var)

pcor2 %>%
  ggplot(aes(x=var, y=delta_sops)) + 
  geom_bar(stat = "identity", fill="#202236") +
  theme_bw() +
  xlab("Predictor") +
  ylab("R Partial") + 
  ggtitle("Partial Correlation of Predictors with Change in SOPS-P") +
  coord_flip()

#avPlots(lm.delta.5, col.lines="#f67c65", id=FALSE, main="Partial Regression of Thalamic GluCEST vs Change in SOPS-P", grid=TRUE, ylab="Change in SOPS-P | Others", xlab="Baseline SOPS-P | Thalamic CEST, Scanner, Sex, Age, Time, Baseline Psych Rx")

# lm.delta.6 <- lm(delta_sops ~ Sex_1M + age_scan + base_sops_p + sips_diff_days + base_rx, final.end)
# Anova(lm.delta.6)
#avPlots(lm.delta.6, terms=~.-Sex_1M-age_scan, col.lines="#6afec4", id=FALSE, main="Added-Variable Plots: Reduced Model")
```

Diagnostics
```{r}
final.end_scaled.pred <- final.end_scaled

final.end_scaled.pred$predicted <- predict(fit.delta.6)
final.end_scaled.pred$residuals <- residuals(fit.delta.6)
head(final.end_scaled.pred)

ggplot(final.end_scaled.pred, aes(x = R_Thalamus_mean, y = delta_sops)) +
  geom_segment(aes(xend = R_Thalamus_mean, yend = predicted), alpha = .2) +
  geom_point(aes(color = residuals)) + #filled point = actual value
  scale_color_gradient2(low = "blue", mid = "white", high = "red") + #red=actual > predicted
...
#first subset df to final tp for each subject
  ...
  mutate(sops_worse = case_when(sops_cat == "remit" ~ 0,
                                sops_cat == "stable" ~ 0,
                                sops_cat == "progress" ~ 1),
         sops_worse_cat = case_when(sops_cat == "remit" ~ "Stable & Remitting",
                                sops_cat == "stable" ~ "Stable & Remitting",
                                sops_cat == "progress" ~ "Worsening"))
...
```{r}
fit.stat.roc<- roc(end.df$sops_worse, fit.stat$fitted, plot=T, col="blue")
fit.stat.roc$auc
fit.stat.pred <- ifelse(fit.stat$fitted > 1/2, "1", "0") #y hat based on majority rule

#plot predictions
cc4 <- scales::seq_gradient_pal("#6afec4", "#f67c65", "Lab")(seq(0,1,length.out=2))
end.df %>%
  ggplot(aes(x=R_Thalamus_mean, y=as_factor(sops_worse))) +
  geom_jitter(height = 0.1, aes(color=fit.stat.pred)) +
  scale_color_manual(values=cc4) +
  xlab("Thalamic GluCEST Contrast") +
  ylab("SOPS-P Progression") + 
  ggtitle("Progression Status by Mean Thalamic GluCEST")

end.df %>%
  ggplot(aes(x=sops_worse_cat, y=R_Thalamus_mean)) +
  geom_dotplot(binaxis='y', stackdir='center', aes(fill=as_factor(sops_worse))) +
  stat_summary(fun.data="mean_sdl", fun.args = list(mult=1), 
                 geom="crossbar", width=0.5) +
  scale_fill_manual(values=cc4) +
  ylab("Thalamic GluCEST Contrast") +
  xlab("SOPS-P Progression Status") + 
  ggtitle("Progression Status by Mean Thalamic GluCEST")
```
...
ppcor::pcor(pcor.df, method="pearson")
```
...
+explanation of (avPlots)[https://stats.stackexchange.com/questions/271314/multiple-regression-avplots-vs-termplot]
