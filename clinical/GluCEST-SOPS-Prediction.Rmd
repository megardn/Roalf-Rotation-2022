...
sops_change_meds.life <- left_join(sops_change_dx, meds.coded, by="observation")
...
```{r}
kruskal.test(sops_change_meds.life$psych.rx_life~sops_change_meds.life$AXIS1_DESC1)
ggplot(sops_change_meds.life) + 
  geom_count(aes(x = factor(AXIS1_DX1), y = as.factor(psych.rx_life))) +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))

sops_change_meds.life %>% 
  group_by(BBLID) %>%
  sample_n(1) %>% #random selection
  ggplot(aes(x = AXIS1_DX1, y = slope.gafc)) +
...
sops_change_meds.life %>% 
  group_by(BBLID) %>%
  sample_n(1) %>% #random selection
  ggplot(aes(x = AXIS1_DX1, y = slope.sops_tot )) +
  geom_point(aes(color=as.factor(psych.rx_life)), show.legend = TRUE) +
  theme(axis.text.x = element_text(angle = 45, hjust=1)) +
