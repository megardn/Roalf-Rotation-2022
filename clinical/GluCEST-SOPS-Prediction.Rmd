...
pacman::p_load(tidyverse, ggplot2, ggpubr, ggrepel, skimr, lubridate, tidyr, data.table, car, ggpubr, corrplot, lme4, lmerTest, report, DescTools, daff, GGally, atable, glmnet, ppcor)
```
...
```{r}
lmer.end <-lmer(sops_p ~ R_Thalamus_mean + scanner + Sex_1M + age_scan + base_dx + psych.rx_life + (1|BBLID), REML = T, data = final.end_scaled)
...
```{r}
lmer.change <- lmer(delta_sops ~ sips_diff_days + R_Thalamus_mean + age_sops + scanner + Sex_1M + base_sops_p + base_rx + (1|BBLID), REML = T, data = final.end_scaled)
...
fit.thal <- lmer(R_Thalamus_mean ~ scanner + sops_worse + (1|BBLID), final.end_scaled)
summary(fit.logit)
Anova(fit.logit)
