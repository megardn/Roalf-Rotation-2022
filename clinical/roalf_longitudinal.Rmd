...
sips.sops_scan <- sips_date %>% mutate(TYPE = as.factor(TYPE))
...
scan1_b <- as.integer(sips.sops_scan.filt$BBLID[clin364_sip1])
scan2_b <- as.integer(sips.sops_scan.filt$BBLID[clin364_sip2])
scan3_b <- as.integer(sips.sops_scan.filt$BBLID[clin364_sip3])
...
``` {r sops}
#filter for SOPS completion - only one data point missing SOPS, just dropping and refiltering
sips.sops_scan.filt <- sips.sops_scan.filt %>% 
  drop_na(P1)

#since dropping NA we need to refilter pre-post subj
##find subjects that have a baseline sops up to 364 days post-7T 
clin364_sop <-  sips.sops_scan.filt  %>% subset(diff_in_days <= 364, select = BBLID) %>% unique()
##find subjects that have a followup sips sops >365 days post-7T 
clin365_sop <- sips.sops_scan.filt %>% subset(diff_in_days >= 365, select = BBLID) %>% unique()
##filter df
sips.sops_scan.filt <- subset(sips.sops_scan.filt, (BBLID %in% clin365_sop$BBLID & BBLID %in% clin364_sop$BBLID))

#sum(is.na(sips.sops_scan.filt[7:25]))#see missing sops scores

sops_scan <- sips.sops_scan.filt %>% mutate(sops_p = rowSums(across(7:11), na.rm=F),
                                  sops_n = rowSums(across(12:17), na.rm=F),
                                  sops_d = rowSums(across(18:21), na.rm=F),
                                  sops_g = rowSums(across(22:25), na.rm=F),
