...
#label SIPS relative to each scan
sips.sops_scan.labeled <- sips.sops_scan.filt %>%
  mutate(scan1.time = case_when(row_number() %in% sip.pre.scan1 ~ "base", 
         ...
         bblid_scan = paste(as.character(BBLID), as.character(scan_7T_date_1))) %>% #making unique ID - temporary until i pull scan ids
  filter_at(vars(scan1.time, scan2.time, scan3.time), any_vars(!is.na(.)))

#label which scans should be use for each ppt
scan1_b <- as.integer(sips.sops_scan.labeled$BBLID[sip.pre.scan1])
scan2_b <- as.integer(sips.sops_scan.labeled$BBLID[sip.pre.scan2])
scan3_b <- as.integer(sips.sops_scan.labeled$BBLID[sip.pre.scan3])
scan1_f <- as.integer(sips.sops_scan.labeled$BBLID[sip.post.scan1])
scan2_f <- as.integer(sips.sops_scan.labeled$BBLID[sip.post.scan2])
scan3_f <- as.integer(sips.sops_scan.labeled$BBLID[sip.post.scan3])

sips.sops_scan.labeled$use_scan1 <- (as.integer(sips.sops_scan.labeled$BBLID) %in% scan1_b & as.integer(sips.sops_scan.labeled$BBLID) %in% scan1_f)
sips.sops_scan.labeled$use_scan2 <- (as.integer(sips.sops_scan.labeled$BBLID) %in% scan2_b & as.integer(sips.sops_scan.labeled$BBLID) %in% scan2_f)
sips.sops_scan.labeled$use_scan3 <- (as.integer(sips.sops_scan.labeled$BBLID) %in% scan3_b & as.integer(sips.sops_scan.labeled$BBLID) %in% scan3_f)

sips.sops_stuff <- subset(sips.sops_scan.labeled, (use_scan2==TRUE | use_scan1==TRUE |use_scan3==TRUE)) #basically follow-up scans add no participants :(

#only keep clinical values that match a scan
sips.sops.labeled <- sips.sops_stuff %>%
...
length(unique(sips.sops.labeled$BBLID))

#plot for fun
ggplot(sips.sops.labeled, aes(x=diff_in_days1, y=BBLID)) + 
  geom_point() +
  geom_vline(xintercept=365) +
  labs(title="SOPS Relative to First 7T Scan", x="Days from 7T Scan") +
  theme(legend.position = "none")

skim(sips.sops.labeled)
```
...
```{r}
#return scanlist
scanlist <- sips.sops.labeled %>% select(BBLID,scan_7T_date_1,scan_scanner_1,scan_SCANID_1,use_scan1, scan_7T_date_2,scan_scanner_2,scan_SCANID_2,use_scan2) %>%
  distinct() %>%
...
sops_scan <- sips.sops.labeled %>% mutate(sops_p = rowSums(across(plist), na.rm=F),
...
Outputting toy dataset to send for Taki for advice in modeling
```{r}
toy <- sops_scan %>%
  arrange(BBLID) %>%
  select(BBLID, bblid_scan, DOSIPS, scan_7T_date_1, age_scan, diff_in_days1, sops_p, sops_n, sops_d, sops_g, sops_tot, scan1.time)
summary(toy)
#if ppt has multiple baseline scans, just select the one closest to scan date
toy.filt <- toy %>%
  group_by(bblid_scan, scan1.time) %>%
  arrange(abs(diff_in_days1)) %>%
  mutate(row = row_number()) %>%
  filter(!(scan1.time=="base" & row > 1)) %>%
  mutate(scan1.time=paste(scan1.time,as.character(row), sep=".")) %>%
  select(-row)%>%
  ungroup()

toy_wide <- pivot_wider(toy.filt, id_cols=c(BBLID, bblid_scan, scan_7T_date_1, age_scan), names_from = scan1.time, values_from=c(DOSIPS, sops_p, sops_n, sops_d, sops_g, sops_tot, diff_in_days1)) %>%
  arrange(BBLID) %>%
  head(.,10) %>%
  select(where(~!all(is.na(.x))))

write.csv(toy_wide, "data/toy_sops_wide.csv", row.names=FALSE, na="")
```
